      ...
      <div className="ot-root">
        ...
        <div className="ot-bar">
          <div className="ot-filters">
            {FILTERS.map(({ value, label, dot }: FilterItem) => {
              const count = getOrders(value).length;
              const selected = filter === value;
              return (
                <button
                  key={value}
                  className={`ot-filter-btn${selected ? " active" : ""}`}
                ...
                >
                  {dot && (
                    <span
                      className="ot-dot"
                      style={{
                        background: selected ? "rgba(255,255,255,0.7)" : dot,
                      }}
                    />
                  )}
                  {label}
