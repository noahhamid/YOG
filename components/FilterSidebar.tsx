...
const CSS = `
  ...
  .fs-wrap { width:232px; flex-shrink:0; font-family:'Sora',sans-serif; }
  .fs-inner {
  ...
  .fs-search-wrap { padding:14px 14px 0; }
  .fs-search { position:relative; display:flex; align-items:center; }
  .fs-search-ico { position:absolute; left:11px; color:#9e9890; pointer-events:none; display:flex; align-items:center; }
  .fs-search-input {
  ...
  .fs-search-clear {
    position:absolute; right:9px; width:18px; height:18px; border-radius:50%; border:none;
  ...
  .fs-clear-banner {
    margin:10px 14px 0; display:flex; align-items:center; justify-content:space-between;
    background:#f6f5f3; border:1px solid #e8e4de; border-radius:9px; padding:8px 12px;
  }
  .fs-clear-label { font-size:11px; font-weight:700; color:#9e9890; }
  .fs-clear-label strong { color:#1a1714; }
  .fs-clear-btn { font-size:11px; font-weight:700; color:#1a1714; background:none; border:none; cursor:pointer; padding:0; font-family:'Sora',sans-serif; transition:color 0.15s; }
  ...
  .fs-radio-dot {
    width:16px; height:16px; border-radius:50%; border:1.5px solid #e8e4de; background:#fff;
    display:flex; align-items:center; justify-content:center; flex-shrink:0; transition:border-color 0.15s;
  }
  .fs-radio-dot::after { content:''; width:7px; height:7px; border-radius:50%; background:#1a1714; transform:scale(0); transition:transform 0.15s; }
  ...
  .fs-check-box {
    width:16px; height:16px; border-radius:4px; border:1.5px solid #e8e4de; background:#fff;
  ...
  .fs-check-input:checked + .fs-check-box { background:#1a1714; border-color:#1a1714; }
  ...
  /* ── Price ── */
  .fs-price-nums { display:flex; justify-content:space-between; font-size:12px; font-weight:600; color:#1a1714; margin-bottom:10px; }
  .fs-price-muted { color:#9e9890; font-weight:500; }
  .fs-price-presets { display:flex; flex-wrap:wrap; gap:6px; margin-bottom:12px; }
  .fs-price-preset {
    padding:4px 10px; border-radius:20px; border:1.5px solid #e8e4de;
    background:#fff; font-size:11px; font-weight:600; color:#9e9890;
    cursor:pointer; font-family:'Sora',sans-serif; transition:all 0.15s;
  }
  .fs-price-preset:hover { border-color:#1a1714; color:#1a1714; }
  .fs-price-preset.active { background:#1a1714; color:#fff; border-color:#1a1714; }
  .fs-range {
  ...
  .fs-range::-webkit-slider-thumb {
    ...
    background:#1a1714; cursor:pointer; box-shadow:0 1px 4px rgba(0,0,0,0.22); transition:transform 0.12s;
  }
  .fs-range::-webkit-slider-thumb:hover { transform:scale(1.2); }
  .fs-range::-moz-range-thumb { width:16px; height:16px; border-radius:50%; background:#1a1714; cursor:pointer; border:none; }

  .fs-size-grid { display:flex; flex-wrap:wrap; gap:6px; }
  .fs-size-pill {
    padding:6px 12px; border-radius:7px; border:1.5px solid #e8e4de; background:#fff;
    font-size:11px; font-weight:700; color:#4a4540; cursor:pointer; font-family:'Sora',sans-serif; transition:all 0.15s;
  }
  ...
  .fs-color-swatch { width:26px; height:26px; border-radius:50%; border:2px solid #e8e4de; cursor:pointer; transition:all 0.15s; }
  .fs-color-swatch:hover { transform:scale(1.1); border-color:#9e9890; }
  .fs-color-swatch.active { border-color:#1a1714; border-width:2.5px; box-shadow:0 0 0 2.5px rgba(26,23,20,0.14); }
  ...
  /* ── Toggle — fixed: pure button, no hidden input ── */
  ...
  .fs-toggle-btn {
    ...
    flex-shrink:0; position:relative; transition:background 0.18s;
  }
  .fs-toggle-btn::after {
  ...
  .fs-toggle-btn.off { background:#e8e4de; }
  .fs-toggle-btn.on  { background:#1a1714; }
  .fs-toggle-btn.on::after { transform:translateX(15px); }
`;
    ...
    <>
      ...
      <div className="fs-wrap">
        <div className="fs-inner">
          ...
          >
            ...
            <div className="fs-price-presets">
              {PRICE_PRESETS.map(([label, min, max]) => (
                <button
                  key={label}
                  className={`fs-price-preset${activePreset?.[0] === label ? " active" : ""}`}
            ...
            <div className="fs-price-nums">
              <span className="fs-price-muted">
                {priceRange[0].toLocaleString()} ETB
              </span>
          ...
          {/* Other — fixed toggles using plain buttons */}
          <Section
            title="Other"
          ...
          >
            <div className="fs-toggle-list">
              <div className="fs-toggle-row">
                ...
                <button
                  className={`fs-toggle-btn ${showNewArrivals ? "on" : "off"}`}
              ...
              <div className="fs-toggle-row">
                ...
                <button
                  className={`fs-toggle-btn ${showOnSale ? "on" : "off"}`}
